...
public class ChangePasswordServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("inputCurrentPassword") != null) {
			inputCurrentPassword = BCrypt.hashpw(request.getParameter("inputCurrentPassword").trim(),
		...
		if (userId != null && inputCurrentPassword != null) {
			if (UserDAO.verifyUserIdAndPassword(userId.toString(), inputCurrentPassword)) {
				...
				request.setAttribute(GlobalConstants.MESSAGE,
						"Mật khẩu đã thay đổi thành công </br>Nhấp vào <a href=\"/Prince/Login\" style=\"text-align: center;\">đây</a> để đăng nhập");
			...
			if (userId != null && isResetPasswordVerified != null) {
				UserDAO.updatePassword(userId.toString(), inputPassword);
				request.setAttribute(GlobalConstants.MESSAGE,
						"Mật khẩu đã thay đổi thành công</br>Nhấp vào <a href=\"/Prince/Login\">đây</a> để đăng nhập");
				RequestDispatcher dispatcher = this.getServletContext()
						.getRequestDispatcher("/WEB-INF/classes/messagetouser.jsp");
				dispatcher.forward(request, response);
			}
